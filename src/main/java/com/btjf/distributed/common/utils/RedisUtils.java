...
public class RedisUtils {
     ...
     */
    public static final String BASIC_REDIS_QUEUE = "basic-information-queue";
    public static final String TEST_REDIS_QUEUE = "web-information-queue";

    private Logger logger = LoggerFactory.getLogger(RedisUtils.class);

    /**
     * 执行器，{@link com.btjf.study.redis.utils.RedisUtils}的辅助类，
    ...
    abstract class Executor<T> {
        ...
        public T getResult() {
            ...
            try {
                ...
                logger.error("Redis execute exception: {}", e.getMessage());
            } finally {
                if (jedis != null) {
                    jedis.close();
                }
     ...
     * @param queue 队列名称
     */
    public Long setList(String queue, String str) {
        return new Executor<Long>(shardedJedisPool) {
            @Override
            Long execute() {
                return jedis.lpush(queue, str);
            }
        }.getResult();
    }
     ...
     * @param queue 队列名称
     */
    public Boolean isHave(String queue) {
        return new Executor<Boolean>(shardedJedisPool) {
            @Override
            Boolean execute() {
                String str = jedis.rpop(queue);
     ...
     * @return 消息内容
     */
    public String getlist(String queue) {
        return new Executor<String>(shardedJedisPool) {
            @Override
            String execute() {
    ...
    /**
     * 存自定义对象---序列化效率没有转JSON快
     */
    public String setObject(String key, Object o) {
        return new Executor<String>(shardedJedisPool) {
            @Override
            String execute() {
                return jedis.set(key.getBytes(), SerializationUtil.serialize(o));
            }
        }.getResult();
    }

    /**
     * 取自定义对象---序列化效率没有转JSON快
     */
    public <T> T getObject(String key, Class<T> cls) {
        return new Executor<T>(shardedJedisPool) {
            @Override
            T execute() {
                return (T) SerializationUtil.deserialize(jedis.get(key.getBytes()));
            }
        }.getResult();
    }
